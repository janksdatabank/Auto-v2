 ...
 */
@Autonomous(name="Blue Corner",group="Auton")
//@Disabled
public class Midnight_Corner_Blue extends OpMode {
    ...
    static final double STRAFE_DIST_LEFT_BLUE                           = -(7 - 3.75);                // The distance when strafing from the left to center column
    static final double STRAFE_DIST_CENTER_BLUE                         = -7;
    static final double STRAFE_DIST_RIGHT_BLUE                          = -(7+3.75);
    static final double STRAFE_DIST_LEFT_RED                            = (7+3.75);                // The distance when strafing from the left to center column
    static final double STRAFE_DIST_CENTER_RED                          = 7;
    static final double STRAFE_DIST_RIGHT_RED                           = (7 - 3.75);
    static final double STRAFE_DIST_PIT_LEFT_BLUE                           = -(6 - 3.75);                // The distance when strafing from the left to center column
    static final double STRAFE_DIST_PIT_CENTER_BLUE                         = -6;
    static final double STRAFE_DIST_PIT_RIGHT_BLUE                          = -(6+3.75);
    static final double STRAFE_DIST_PIT_LEFT_RED                            = (4);                // The distance when strafing from the left to center column
    static final double STRAFE_DIST_PIT_CENTER_RED                          =  (4+3.75);
    static final double STRAFE_DIST_PIT_RIGHT_RED                           = (4 + 7.5);
    ...
    static final double RIGHT_COL_DIST_BLUE                         = 23 + 8.5;
    static final double LEFT_COL_DIST_BLUE                          = 23 - 2.5;
    static final double ANGLE_OF_BOX_BLUE                           = -180;              // Angle to face the blue cryptobox
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case 3:
                ...
                if (Center) {
                    distance = STRAFE_DIST_CENTER_RED;
                }
                else if (Left) {
                    distance = STRAFE_DIST_LEFT_RED;
                }
                else  {
                    distance = STRAFE_DIST_RIGHT_RED;
                }

                if (ALLIANCE_COLOR.equals("Red")){
                    if (encoderIMUdrive(-16, -.3, 0)) {
                        nextState();//state++;
                    }
                } else{
                    if (encoderIMUdrive(16, .3, 0)) {
            ...
            case 5:
                if (Center) {
                    strafeDist = STRAFE_DIST_CENTER_BLUE;
                }
                else if (Left) {
                    strafeDist = STRAFE_DIST_LEFT_BLUE;
                }
                else  {
                    strafeDist = STRAFE_DIST_RIGHT_BLUE;
                }

                RobotLog.ii("STATE", "Strafing to Pictograph Column");
                if (angleDrive(strafeDist, STRAFE_SPEED, STRAFE_RIGHT, boxAngle)) {
                    nextState();
                }
                break;
            ...
            case 21:
                if (Center) {
                    strafeDist = STRAFE_DIST_PIT_CENTER_RED;
                }
                else if (Left) {
                    strafeDist = STRAFE_DIST_PIT_LEFT_RED;
                }
                else  {
                    strafeDist = STRAFE_DIST_PIT_RIGHT_RED;
                }

                RobotLog.ii("STATE", "Strafing to Pictograph Column");
                if (angleDrive(strafeDist, STRAFE_SPEED, STRAFE_LEFT, boxAngle)) {
                    nextState();
                }
                break;
            case 10:
                if (encoderIMUdrive(16, FAST_DRIVE_SPEED, boxAngle)) {
                    nextState();
                }
            ...
            case 14:
                ...
                backBrown = false;
                backGrey = false;
                if (encoderIMUdrive(((encoderCount/COUNTS_PER_INCH)-16), -FAST_DRIVE_SPEED, boxAngle)) {
                    nextState();
                }
                break;
            case 15:
                if (angleDrive(4, STRAFE_SPEED, STRAFE_RIGHT, boxAngle)) {
                    nextState();
                } break;
            ...
            case 17:
                ...
                RobotLog.ii("OUTPUT_INFO", "FL Current Encoder: %s", frontLeftMotor.getCurrentPosition());
                RobotLog.ii("OUTPUT_INFO", "FR Current Encoder: %s", frontRightMotor.getCurrentPosition());
                RobotLog.ii("OUTPUT_INFO", "RL Current Encoder: %s", rearLeftMotor.getCurrentPosition());
                RobotLog.ii("OUTPUT_INFO", "RR Current Encoder: %s", rearRightMotor.getCurrentPosition());

                alignArm.setPosition(0);
                if (PIDdrive(3, -FAST_DRIVE_SPEED)){
                    stopStateMachine();
                }
    ...
    public boolean encoderIMUdrive(double distance, double speed, double angleTarget) {
        ...
        angles  = imu.getIntegratedZValue();
        angleCurrent = angles;
    ...
    public boolean turn(double angleTarget) {
        ...
        if ((Math.abs(angleError) > HEADING_THRESHOLD)) {
            ...
            if (drivePower >= 0){
                drivePower = Math.max(drivePower, .1);
            } else{
                drivePower = Math.min(drivePower, -.1);
            }
